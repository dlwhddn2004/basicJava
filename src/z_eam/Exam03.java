	...
	System.out.println(y += 10 - x++);	답 : y = y + 10 -x++  답 :13
	System.out.println(x+=2); 답 : 5
	System.out.println( !('A' <= c && c <='Z') );답 : false
	System.out.println('C'-c);	<= c ='A' 가 되니 65 그리고 c 는 67 답:  2
	System.out.println('5'-'0'); 답: 5 
	System.out.println(c+1);  답:66  단항연산자 이므로 c = 'A' -> 65 즉 65+ 1 =66
	System.out.println(++c);  67 B 단항연산자와 달리 이향 연산하므로  선위형이기때문에  답 =B 
	System.out.println(c++);  67 후위형 이기때문에 참조된 후에 더해주므로 아직 B 답 :B
	System.out.println(c); 68  전 코드 (후위형)으로 인해 한단계 업 68 즉 답 : C
	}
	}
	...
	[실행결과]
	13
	 답 : int numOfBucket = numOFApples%sizeOfBucket == 0 ? 
	...
	public static void main(String[] args) {
	int num = 10;
	System.out.println(  (1)  ); (답) num < 0 ? "음수" :(num>0 ? "양수" :" 0")
	}
	}
	...
	답 :(int)(num /100.0) *100 or num - num%100
	...
	답 :(int)(num /10.0) *10+1  (num/10) *10+1  --> 어차피 인트형이기때문에 형변환 할필요가없군
	}
	}
	[실행결과]
	양수
	...
	public static void main(String[] args) {
	int num = 24;
	System.out.println(  (1)  );  답 : 10-num%10
	}
	}
	...
	// 문제 8			
		...
		//문제 9
		char ch1 = '가';
		boolean b1 = ('a'<=ch1 && ch1 <='z') || ('A'<=ch1 && ch1<='Z') || ('0'<=ch && ch<='9');
		System.out.println(b1);
		
		//문제 10
		char ch3 = '가';
		char ch4 = 'A' <=ch3 && ch3 < 'Z' ? (char)(ch +32) :ch3;
		
		System.out.println("ch:"+ch3);
		System.out.println("ch to lowerCase:"+ch4);
		*/
