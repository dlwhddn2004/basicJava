...
public class ExceptionHandling {
	public static void main(String[] args) {
		 ...
		 * - [RuntimeException 클래스와 그 자식들을 제외한 ] Exception 클래스의 자식들은 예외 처리가 강제된다.( checked예외)
		 * 
		 * 
		 * << 예외처리(try-catch) >>
		...
		}catch(ArithmeticException | IndexOutOfBoundsException e){ // | 는 예외 처리할 다른 클래스 등 으로 표시
			e.printStackTrace();
		...
		//Exception = 무조건 발생되는 에러 vs 개발자의 코드 실수로 발생하는 에러 = runtimeException
...
//		System.out.println(10/0 ); 에러발생위치 내가 만든 메소드에 에러
		try {
			new FileInputStream("");
